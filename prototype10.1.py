...
def main():
    ...
    if sintomas:
        ...
        for sintoma in sintomas:
            ...
            dados_paciente_np = [dados_paciente_dict[coluna]
    ...
    if diagnosticos_selecionados:
        ...
        for diagnostico, sintomas_selecionados in diagnosticos_selecionados.items():
            sintomas_formatados = [formatar_diagnostico(
                sintoma) for sintoma in sintomas_selecionados]
...
def salvar_arquivo(iniciais_usuario, diagnosticos_selecionados, sintomas, observacoes, avaliacao, tempo_decorrido):
    try:
        with open("resultado_avaliacoes.csv", mode="a", newline="") as file:
            ...
            if file.tell() == 0:
                writer.writerow(['Iniciais do Usuário', 'Diagnóstico', 'Sintomas',
                                'Observações', 'Avaliação', 'Tempo de Seleção (s)'])

            for diagnostico, sintomas_diagnostico in diagnosticos_selecionados.items():
                sintomas_formatados = [formatar_diagnostico(
                    sintoma) for sintoma in sintomas_diagnostico]
                writer.writerow([iniciais_usuario, diagnostico, ", ".join(
                    sintomas_formatados), observacoes, avaliacao, tempo_decorrido])
