...
def main():
    ...
    atributos = [coluna for coluna in df.columns if coluna !=
                 'diagnostico_de_Enfermagem']
    ...
    if sintomas:
        ...
        for sintoma in sintomas:
            ...
            indices_diagnosticos = [indice for indice, probabilidade in enumerate(
                probas) if probabilidade > limiar_probabilidade]
        ...
        for diagnostico, sintoma in diagnosticos_associados:
            ...
            sintoma_selecionado = sintoma_selecionado.replace(
                "-", " ")  # Substituir traços por espaços
            sintoma_selecionado = sintoma_selecionado.title()  # Capitalizar palavras
            cuidado_key = f"Cuidado_{diagnostico}"
            escolhido = st.checkbox(
                f"Diagnóstico: {diagnostico.replace('_', ' ')}. Característica Definidora: {sintoma_selecionado}")
            if escolhido:
    ...
    diagnostico_personalizado = st.text_area(
        "Se desejar, escreva um diagnóstico personalizado:", "")

    if diagnostico_personalizado:
        diagnosticos_selecionados["Personalizado"] = [
    ...
    if diagnosticos_selecionados:
        ...
        for diagnostico, sintomas_selecionados in diagnosticos_selecionados.items():
            sintomas_concatenados = ", ".join(sintomas_selecionados)
            st.markdown(
                f"<span style='font-size:18px'>{diagnostico.replace('_', ' ')}. Características definidoras: {sintomas_concatenados}</span>", unsafe_allow_html=True)
            if diagnostico in cuidados_df.columns:
                ...
                if cuidados_relacionados:
                    ...
                    for cuidado in cuidados_relacionados.split('\t'):
                        st.markdown(
                            "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;• " + cuidado, unsafe_allow_html=True)

        observacoes = st.text_area(
            "Observações gerais sobre os diagnósticos selecionados:", "")
        ...
        if st.button("Salvar Avaliação"):
            ...
            tempo_decorrido = round(fim_selecao - inicio_selecao, 2)
            salvar_arquivo(iniciais_usuario, diagnosticos_selecionados,
                           sintomas, observacoes, avaliacao, tempo_decorrido)
    ...
    try:
        with open("resultado_avaliacoes.csv", mode="a", newline="") as file:
            ...
            for diagnostico, sintomas_diagnostico in diagnosticos_selecionados.items():
                writer.writerow([iniciais_usuario, diagnostico, ", ".join(
                    sintomas_diagnostico), observacoes, avaliacao, tempo_decorrido])
